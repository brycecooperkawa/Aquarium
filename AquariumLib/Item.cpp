 ...
 * @param m mirror flag
 */
void Item::SetBitmap(bool m)
{
    //if the passed parameter is true it flips the fish image
    if(m==true)
    {
        mItemBitmap = make_unique<wxBitmap>(mItemImage->Mirror());
    }

    //if the passed parameter is false it unflips the fish image
    if(m==false)
    {
        mItemBitmap = make_unique<wxBitmap>(*mItemImage);
    }
...
/**
 * Evolves a Magikarp into a Gyarados
 */
void Item::Evolve()
{
    const wstring GyaradosImageName = L"images/gyarados.png";
    mItemImage->LoadFile(GyaradosImageName);
}